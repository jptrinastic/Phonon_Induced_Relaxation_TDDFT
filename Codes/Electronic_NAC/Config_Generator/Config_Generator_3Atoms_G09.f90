! This code generates posX files containing center-of-mass-weighted (COM) coordinates for each time step from the BOMD trajectory calculated from Gaussian09, where X is the Xth timestep of the molecular dynamics simulation.
! Works with Gaussian09.D01
...
!        2) 'MD_G09' file: Output file from BOMD simulation using Gaussian09.D01
program readBOMD
implicit none
...
real :: mass_tot, mass1, mass2, mass3, garbage

open(6,file='input',status='old')
open(7,file='MD_G09',status='old')
...
! Read in coordinates at each time step from MD_G09
k=istep

do while (k.gt.0)
 read (7,'(1X,A65)') t
! Search for each new configuration for sequential time steps
 if (t=='Center     Atomic      Atomic             Coordinates (Angstroms)') then
   read(7,*)
   read(7,*)
   do i=1,nat
    read(7,*) garbage, garbage, garbage, coord(1,i,istep-k+1),coord(2,i,istep-k+1), coord(3,i,istep-k+1)
   enddo
   k=k-1
 endif
enddo
...
open(9,file='COM',status='replace')
...
! Write out COM-shifted coordinates for each time step X to posX
 ...
 do k=1,natom1
    ...
    write(8,'(1X,A2,1X,6(F12.5,1X))') atom1,coord(1,k,i),coord(2,k,i),coord(3,k,i)
 enddo
 do k=natom1+1,natom1+natom2
    coord_sh(1,k,i) = coord(1,k,i)-COM_x(i)
    coord_sh(2,k,i) = coord(2,k,i)-COM_y(i)
    coord_sh(3,k,i) = coord(3,k,i)-COM_z(i)
    write(8,'(1X,A2,1X,6(F12.5,1X))') atom2,coord(1,k,i),coord(2,k,i),coord(3,k,i)
 enddo
 do k=natom1+natom2+1,natom1+natom2+natom3
    coord_sh(1,k,i) = coord(1,k,i)-COM_x(i)
    coord_sh(2,k,i) = coord(2,k,i)-COM_y(i)
    coord_sh(3,k,i) = coord(3,k,i)-COM_z(i)
    write(8,'(1X,A2,1X,6(F12.5,1X))') atom3,coord(1,k,i),coord(2,k,i),coord(3,k,i)
 enddo
 close(8)
enddo
