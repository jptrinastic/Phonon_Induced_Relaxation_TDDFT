...
// ** Use TDDFT NAC code to calculate TDDFT NAC (Tavernelli et al 2009) for each pair of adjacent time steps using output_ovlap data.  Then use final code to converge cumulative average of the square of the TDDFT coupling matrix with respect to number of time steps from MD run- this will be Vab^2 for relaxation dynamics**
...
int main()
{
  ...
  // NOTE: ONLY DEFINED THROUGH f-ORBITALS AT THIS POINTS - need to add more for higher angular momenta
  ...
  double ovlap_mat[num_elevels][num_elevels];
  ...
  for (int i = elevels_min-1; i < elevels_max; i++)
    {
      for (int j = elevels_min-1; j < elevels_max; j++)
	{
	  ovlap_mat[i][j] = 0.0;
	  for (int l = 0; l < nprim1; l++)
	      ...
	      for (int m = 0; m < nprim2; m++)
		{
		  ...
		  ovlap_mat[i][j] = ovlap_mat[i][j] + pow(sqrt(pi/(prim_exp1[l]+prim_exp2[m])),3)*ov_gauss*ov_x*ov_y*ov_z*mo_coeff1[i][l]*mo_coeff2[j][m];
		}
	    }
	}
    }
  ...
  for (int i = elevels_min-1; i < elevels_max; i++)
    {
      for (int j = elevels_min-1; j < elevels_max; j++)
	{
	  out_ovlap_matrix << scientific << setprecision(6) << ovlap_mat[i][j] << " ";
	}
  ...
  for (int i = elevels_min-1; i < elevels_max; i++)
    {
      for (int j = elevels_min-1; j < elevels_max; j++)
        {
	  coupling_mat[i][j] = hbar_evfs*(ovlap_mat[i][j] - ovlap_mat[j][i])/(2*delta_t);
	}
    }
