%This code treats the nuclei classically such that the Franck-Condon-weighted
%DOS simplifies to a simple DOS (nuclear wave function overlap is 1)
    % Assumptions: 1) normal modes of ground state are used
    %              2) harmonic approximation
    %INPUT - 1) transition energy levels (energy_pop), 2) phonon modes (modes_cm)
    %--Can also apply gaussian or lorentzian smearing to lineshape
%OUTPUT - 1) DeltaG(t), 2) DeltaG(0), 3) Lineshape function 4) F(wab) matrix
%OUTPUT - Key output is dos_values.txt, which is input for relaxation dynamics code
...
e_max = 27; % Last excited state index
...
modes_cm = load('modes_cm.txt'); % modes in cm^-1 from phonon mode calculation
...
% Limit for cutting off Fourier transform result for Gaussian smearing and fittin g to save time (in eV)
ft_max_energy = 5; 
...
%Create integrand for Fourier transform - for DOS this is just mode frequency multipled by time
full_G = zeros(num_ts,1);
exp_full_G = zeros(num_ts,1);
        for t=1:num_ts;
            full_G(t) = 0.0;
            for l=1:nmodes;
                full_G(t) = full_G(t) + exp(-1j*(2*pi*modes_nu(l))*time_steps(t));
            end;
            exp_full_G(t) = full_G(t);
        end;
...
ft_expDeltaGt_evfs = zeros(num_ts,1);
ft_expDeltaGt_evfs(:,1) = ft_expDeltaGt(:,1)/hbar;
...
ft_expDeltaGt_evfs_cutoff = zeros(num_cutoff,1);
for t = 1:num_cutoff;
    ft_expDeltaGt_evfs_cutoff(t,1) = ft_expDeltaGt_evfs(t,1);
end;

ft_expDeltaGt_evfs_cut_FINAL = zeros(num_cutoff,1);
...
ft_expDeltaGt_evfs_cut_gauss = zeros(num_cutoff,1);
ft_expDeltaGt_evfs_cut_gauss_norm = zeros(num_cutoff,1);
ft_expDeltaGt_evfs_cut_lorentz = zeros(num_cutoff,1);
ft_expDeltaGt_evfs_cut_lorentz_norm = zeros(num_cutoff,1);
...
if (smearing_l==1)
    lorentz_sum = zeros(num_cutoff,1);
        ...
        for j=1:num_cutoff;
            lorentz_sum(j,1) = 0.0;
            for k=1:num_cutoff;
                    ft_expDeltaGt_evfs_cut_lorentz(j,1) = ft_expDeltaGt_evfs_cut_lorentz(j,1) + ft_expDeltaGt_evfs_cutoff(k,1)*((l_smearing/2)/((l_smearing/2)^2+((x_axis_ev_cut(j)-x_axis_ev_cut(k))^2)));
                    lorentz_sum(j,1) = lorentz_sum(j,1) + ((l_smearing/2)/((l_smearing/2)^2+((x_axis_ev_cut(j)-x_axis_ev_cut(k))^2)));
            end
            ft_expDeltaGt_evfs_cut_lorentz_norm(j,1) = ft_expDeltaGt_evfs_cut_lorentz(j,1)/lorentz_sum(j,1);
        end
...
if (smearing_g==1)
    gaussian_sum = zeros(num_cutoff,1);
        for j=1:num_cutoff;
            ...
            for k=1:num_cutoff;
                    ft_expDeltaGt_evfs_cut_gauss(j,1) = ft_expDeltaGt_evfs_cut_gauss(j,1) + ft_expDeltaGt_evfs_cutoff(k,1)*exp(-((x_axis_ev_cut(j)-x_axis_ev_cut(k))^2)/(2*g_smearing*g_smearing));
                    gaussian_sum(j,1) = gaussian_sum(j,1) + exp(-((x_axis_ev_cut(j)-x_axis_ev_cut(k))^2)/(2*g_smearing*g_smearing));
            end
            ft_expDeltaGt_evfs_cut_gauss_norm(j,1) = ft_expDeltaGt_evfs_cut_gauss(j,1)/gaussian_sum(j,1);
        end
...
for i=1:band_comb;
    ls_spline_value_one_row(1,i) = interp1(x_axis_ev_cut(:,1),ft_expDeltaGt_evfs_cut_FINAL(:,1),band_diff_one_row(i),'spline');
end;
...
%modes_nu
dlmwrite('modes_nu_sig.txt',modes_nu,'precision',5,'delimiter',' ')
...
dlmwrite('dos_values.txt',ls_spline_value_mat,'precision',5,'delimiter',' ')
